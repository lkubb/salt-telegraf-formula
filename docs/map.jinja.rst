...
We will see a quick setup to configure the ``telegraf`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``telegraf`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``telegraf`` formulas under ``/srv/salt/telegraf/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/telegraf/parameters/dns:domain/
    ln -s dns:domain /srv/salt/telegraf/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/telegraf/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/telegraf/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``telegraf`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``telegraf`` formulas under ``/srv/salt/telegraf/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/telegraf/parameters/roles
...
We create a configuration for the role ``telegraf/server`` in ``/srv/salt/telegraf/parameters/roles/telegraf/server.yaml``:
...
We create another configuration for the role ``telegraf/client`` in ``/srv/salt/telegraf/parameters/roles/telegraf/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles telegraf/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles telegraf/client
...
.. code-block:: console
    ...
    mv /srv/salt/telegraf/parameters/dns:domain/  '/srv/salt/telegraf/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-telegraf-as-json:
      test.nop:
        - name: {{ telegraf | json }}
...
Here is an example based on `template-formula/telegraf/config/file.sls`_:
    ...
    telegraf-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='telegraf-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ telegraf.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``telegraf`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ telegraf['winner'] }}
...
.. _template-formula/telegraf/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/telegraf/config/file.sls
